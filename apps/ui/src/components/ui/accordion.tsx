...
const AccordionItem = forwardRef<HTMLDivElement, AccordionItemProps>(
  ({ value, index, disabled, children, className, ...props }, ref) => {
    ...
    const shape = useShape();
    const grouped = groupCtx?.grouped === true;
    const registerItem = groupCtx?.registerItem;
    const registerFullItem = groupCtx?.registerFullItem;

    const isOpen = grouped
    ...
    useEffect(() => {
      if (grouped && index !== undefined && registerItem) {
        registerItem(index, triggerRef.current);
        return () => registerItem(index, null);
      }
    }, [grouped, index, registerItem]);

    // Register full item element for expanded background measurement
    useEffect(() => {
      if (grouped && index !== undefined && registerFullItem) {
        if (isOpen) {
          registerFullItem(index, internalRef.current);
        } else {
          registerFullItem(index, null);
        }
        return () => registerFullItem(index, null);
      }
    }, [grouped, index, isOpen, registerFullItem]);
    ...
    return (
      <AccordionItemContext.Provider value={{ index, value, isOpen, triggerRef }}>
        <AccordionPrimitive.Item
          ...
          className={cn(!grouped && "relative", className)}
          {...props}
        >
...
const AccordionTrigger = forwardRef<HTMLButtonElement, AccordionTriggerProps>(
  ({ children, className, indicator, ...props }, ref) => {
