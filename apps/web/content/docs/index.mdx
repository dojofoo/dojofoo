...
`install` detects your coding agent (via env vars like `CLAUDECODE=1`) and wires the `/kata` slash command into it, `add` pulls in a dojo, and `/kata` puts you in front of your first failing test with the sensei watching. Pass `--claude`, `--opencode`, `--codex`, or `--gemini` if you need to override detection.

## Where to go next

- **Getting Started** — install the CLI, scaffold your first dojo project.
- **Commands** — the `npx dojos` CLI surface: `install`, `add`, `kata`.
