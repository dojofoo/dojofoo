...
```sh
npx dojofoo add effect-ts            # install a dojo into the current repo
npx dojofoo add pydantic-agents      # multiple dojos coexist
claude /kata                  # start practicing
```

Each dojo is a workspace package under `dojos/` in the [main repo](https://github.com/dojofoo/dojofoo), so it's easy to see exactly what your agent is being told.
